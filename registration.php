    ...
    <body>
        <?php
            ...
            if($_SERVER['REQUEST_METHOD'] == "POST" && $_REQUEST['button'] == "Submit") {
                ...
                if(empty($_POST['fname']) || empty($_POST['lname']) || empty($_POST['gender']) || empty($_POST['e-mail']) || empty($_POST['img']) || empty($_POST['pass']) || empty($_POST['cpass'])  || empty($_POST['location'])  || empty($_POST['phone'])) {
                    ...
                    for($i = 0; $i< count($data_filter)-1; $i++) {
                        ...
                        if($notavailable == "")      
                        {             
                            $details = array('firstname' => $firstname, 'lastname' => $lastname, 'gender' => $gender, 'email' => $email, 'image' => $img, 'location' => $location ,'phone' => $phone);
